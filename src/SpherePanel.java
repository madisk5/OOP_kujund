import javax.swing.*;
import java.awt.*;

public class SpherePanel extends JPanel {
    private JTextField radiusField;
    private JLabel surfaceAreaLabel;
    private JLabel circumferenceLabel;
    private JLabel volumeLabel;
    private JLabel inputDataLabel;
    private SphereController controller;
    private JButton clearButton;
    private JButton calculateButton;

    public SpherePanel() {
        setLayout(new GridBagLayout());
        GridBagConstraints gbc = new GridBagConstraints();
        gbc.insets = new Insets(5, 5, 5, 5);
        gbc.anchor = GridBagConstraints.WEST;
        gbc.fill = GridBagConstraints.HORIZONTAL;

        gbc.gridx = 0;
        gbc.gridy = 0;
        gbc.gridwidth = 2;
        JLabel titleLabel = new JLabel("Kera Arvutused");
        titleLabel.setFont(new Font("Arial", Font.BOLD, 16));
        add(titleLabel, gbc);
        gbc.gridwidth = 1;


        gbc.gridx = 0;
        gbc.gridy = 1;
        add(new JLabel("Raadius:"), gbc);
        radiusField = new JTextField(10);
        radiusField.setEnabled(false);
        gbc.gridx = 1;
        add(radiusField, gbc);

        gbc.gridx = 0;
        gbc.gridy = 2;
        add(new JLabel("Pindala:"), gbc);
        surfaceAreaLabel = new JLabel();
        gbc.gridx = 1;
        add(surfaceAreaLabel, gbc);

        gbc.gridx = 0;
        gbc.gridy = 3;
        add(new JLabel("Ümbermõõt:"), gbc);
        circumferenceLabel = new JLabel();
        gbc.gridx = 1;
        add(circumferenceLabel, gbc);

        gbc.gridx = 0;
        gbc.gridy = 4;
        add(new JLabel("Ruumala:"), gbc);
        volumeLabel = new JLabel();
        gbc.gridx = 1;
        add(volumeLabel, gbc);

        // Sisesta andmed väljad
        gbc.gridx = 0;
        gbc.gridy = 5;
        gbc.gridwidth = 2;
        inputDataLabel = new JLabel();
        add(inputDataLabel, gbc);

        // Arvuta nupp
        gbc.gridx = 0;
        gbc.gridy = 6;
        gbc.gridwidth = 2;
        gbc.anchor = GridBagConstraints.CENTER;
        calculateButton = new JButton("Arvuta");
        calculateButton.setEnabled(false); // Disable initially
        add(calculateButton, gbc);

        // Tühjenda väljad nupp
        gbc.gridx = 0;
        gbc.gridy = 7;
        gbc.gridwidth = 2;
        gbc.anchor = GridBagConstraints.CENTER;
        clearButton = new JButton("Tühjenda väljad");
        clearButton.setEnabled(false); // Disable initially
        add(clearButton, gbc);

        controller = new SphereController(this);
        calculateButton.addActionListener(controller);
        clearButton.addActionListener(e -> clearFields());
    }

    public String getRadiusText() {
        return radiusField.getText();
    }

    public void setSurfaceArea(String surfaceArea) {
        surfaceAreaLabel.setText(surfaceArea);
    }

    public void setCircumference(String circumference) {
        circumferenceLabel.setText(circumference);
    }

    public void setVolume(String volume) {
        volumeLabel.setText(volume);
    }

    public void setInputData(String data) {
        inputDataLabel.setText("<html>Algandmed<br>Raadius: " + data + "</html>");
    }

    public void clearFields() {
        radiusField.setText("");
        surfaceAreaLabel.setText("");
        circumferenceLabel.setText("");
        volumeLabel.setText("");
        inputDataLabel.setText("");
    }

    @Override
    public void setEnabled(boolean enabled) {
        super.setEnabled(enabled);
        radiusField.setEnabled(enabled);
        calculateButton.setEnabled(enabled);
        clearButton.setEnabled(enabled);
    }

    public void setFilePath(String filePath) {
        Container parent = getParent();
        while (parent != null && !(parent instanceof MainPanel)) {
            parent = parent.getParent();
        }
        if (parent != null) {
            ((MainPanel) parent).setFilePath(filePath);
        }
    }
}
